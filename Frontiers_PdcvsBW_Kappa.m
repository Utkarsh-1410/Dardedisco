...
% bandwidth (in Hz)
BW = logspace(7,9,20);
tau = ones(1,length(BW))./BW;
% Noise (in Watts)
noise = kB*Ts*BW;
% Range resolution (m)
delr = c*ones(1,length(BW))./(2*BW);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Pd = zeros(length(Kappas),length(BW));
Throughput = zeros(length(Kappas),length(BW));
...
for q = 1:length(Kappas)
    ...
    for k = 1:length(BW)
        ...
        delrk= delr(k)/sqrt(1-(L^2/(4*kappa_m^2)));
    ...
    %             scatter(x0,y0,'bx'); hold on;
            ...
            for j = 1:Nc
                ...
                flag_range = 1;
                if abs(rc-r) < delr(k)/sqrt(1-(L^2/(4*kappa_m^2)))
                    flag_range=0;
                end
...
semilogx(BW*1e-6,Pd(1,:),'rv-','LineWidth',2,'MarkerSize',10);hold on;
semilogx(BW*1e-6,Pd(2,:),'ro-','LineWidth',2,'MarkerSize',10);hold on;
semilogx(BW*1e-6,Pd(3,:),'r*-','LineWidth',2,'MarkerSize',10);hold on;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% STOCHASTIC GEOMETRY FORMULATIONS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Pdc = zeros(length(Kappas),length(BW));
Throughput = zeros(length(Kappas),length(BW));
SNR = Pdc;
...
for q = 1:length(Kappas)
    ...
    for i = 1:length(BW)
        ...
        SNR_Nr = threshold*noise(i)*(kappa_m^4);
        SNR_Dr = P_tx*H0*sigma_m_avg*B0*epsilon;
        SNR_const = SNR_Nr/SNR_Dr;
        SNR(i) = SNR_const;
        
        SCR_Nr = threshold*rho*c*tau(i)*(kappa_m^2)*sigma_c_avg;
        SCR_Dr = B0*epsilon*(sigma_m_avg+(threshold*sigma_c_avg)*(kappa_m+sqrt(kappa_m^2-L^2)));
        ...
        tp_const = (2*pi*kappa_m) - ((3*pi*L*L)/(8*kappa_m));
        delrk= delr(i)/sqrt(1-(L^2/(4*kappa_m^2)));
        tp_const = tp_const/delrk;
        tp_const = tp_const*(1-epsilon);
...
semilogx(BW*1e-6,Pdc(1,:),'bv-','LineWidth',2,'MarkerSize',10); hold on;
semilogx(BW*1e-6,Pdc(2,:),'bo-','LineWidth',2,'MarkerSize',10); hold on;
semilogx(BW*1e-6,Pdc(3,:),'b*-','LineWidth',2,'MarkerSize',10); hold on;
grid on;
set(gca,'Fontsize',16);
set(gca,'Ylim',[0 1]);
xlabel('Bandwidth $BW$ ($\times 10^6$ Hz)','interpreter','latex','Fontsize',20,'FontWeight','bold');
...
% saveas(gcf,'PdvsBW_Kappa','fig');
% saveas(gcf,'PdcvsBW_Kappa','png');

% figure(2);
subplot(1,2,2);
semilogx(BW*1e-6,Throughput(1,:),'rv-','LineWidth',2,'MarkerSize',10);hold on;
semilogx(BW*1e-6,Throughput(2,:),'bo-','LineWidth',2,'MarkerSize',10);hold on;
semilogx(BW*1e-6,Throughput(3,:),'k*-','LineWidth',2,'MarkerSize',10);hold on;
grid on;
set(gca,'Fontsize',16);
xlabel('Bandwidth $BW$ ($\times 10^6$ Hz)','interpreter','latex','Fontsize',20,'FontWeight','bold');
...
saveas(gcf,'ResultsvsBW_Kappa','fig');
saveas(gcf,'ResultsvsBW_Kappa','png');