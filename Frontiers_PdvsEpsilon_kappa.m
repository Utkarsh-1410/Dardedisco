...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Pd = zeros(length(epsilon),length(Kappas));
Throughput = zeros(length(epsilon),length(Kappas));

for q = 1:length(Kappas)
    ...
    for k = 1:length(epsilon)
        ...
        for i = 1:i_max
            ...
            % received power
            S = (P_tx*H0*sigma_m*B0*epsilon(k))/(kappa_ch^4);
            % beta
    ...
    %             scatter(x0,y0,'bx'); hold on;
            ...
            slope_t1 = tan(theta_t - Delta_theta(q)/2);
            slope_t2 = tan(theta_t + Delta_theta(q)/2);
            ...
            for j = 1:Nc
                ...
                if flag_beamwidth==0
                    ...
                    % received power
                    C = C + (P_tx*H0*sigma_c*B0*epsilon(k))/(R_tc^2*R_rc^2);          
                    n1=n1+1;
...
figure(1);
subplot(1,2,1);
plot(epsilon,Pd(1,:),'rv-','LineWidth',2,'MarkerSize',10);hold on;
plot(epsilon,Pd(2,:),'ro-','LineWidth',2,'MarkerSize',10);hold on;
plot(epsilon,Pd(3,:),'r*-','LineWidth',2,'MarkerSize',10);hold on;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% STOCHASTIC GEOMETRY FORMULATIONS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Pdc = zeros(length(Kappas),length(epsilon));
Throughput = zeros(length(Kappas),length(epsilon));
SNR = Pdc;
...
for q = 1:length(Kappas)
    ...
    for i = 1:length(epsilon)
        ...
        SNR_Dr = P_tx*H0*sigma_m_avg*B0*epsilon(i);
        SNR_const = SNR_Nr/SNR_Dr;
        SNR(i) = SNR_const;
        
        SCR_Nr = threshold*rho*c*tau*(kappa_m^2)*sigma_c_avg;
        SCR_Dr = B0*epsilon(i)*(sigma_m_avg+(threshold*sigma_c_avg)*(kappa_m+sqrt(kappa_m^2-L^2)));
...
% figure(1);
plot(epsilon,Pdc(1,:),'bv-','LineWidth',2,'MarkerSize',10); hold on;
plot(epsilon,Pdc(2,:),'bo-','LineWidth',2,'MarkerSize',10); hold on;
plot(epsilon,Pdc(3,:),'b*-','LineWidth',2,'MarkerSize',10); hold on;
grid on;
set(gca,'Fontsize',16);
set(gca,'Ylim',[0 1]);
xlabel('Search/Serve Duty cycle $\epsilon$ (n.u)','interpreter','latex','Fontsize',20,'FontWeight','bold');
...
legend({'\kappa_m = 30m (M.C)','\kappa_m = 40m (M.C)','\kappa_m = 50m (M.C)','\kappa_m = 30m (S.G)','\kappa_m = 40m (S.G)','\kappa_m = 50m (S.G)'},'Location','best','NumColumns',2);

% saveas(gcf,'PdvsEpsilon_Kappa','fig');
% saveas(gcf,'PdcvsEpsilon_Kappa','png');

% figure(2);
subplot(1,2,2);

plot(epsilon,Throughput(1,:),'rv-','LineWidth',2,'MarkerSize',10);hold on;
plot(epsilon,Throughput(2,:),'bo-','LineWidth',2,'MarkerSize',10);hold on;
plot(epsilon,Throughput(3,:),'k*-','LineWidth',2,'MarkerSize',10);hold on;
grid on;
set(gca,'Fontsize',16);
xlabel('Search/Serve Duty Cycle $\epsilon$ (n.u)','interpreter','latex','Fontsize',20,'FontWeight','bold');
ylabel('Network Throughput $\Upsilon$ (bps)','interpreter','latex','Fontsize',20,'FontWeight','bold');
% ylabel('Bistatic radar detection coverage probability','Fontsize',14,'FontWeight','bold');
legend({'\kappa_m = 30m','\kappa_m = 40m','\kappa_m = 50m'},'Location','best','NumColumns',2);

saveas(gcf,'ResultsvsEpsilon_Kappa','fig');
saveas(gcf,'ResultsvsEpsilon_Kappa','png');