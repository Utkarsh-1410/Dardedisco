...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Pd = zeros(length(rho),length(Kappas));
Throughput = 0*Pd;
...
for q = 1:length(rho)
    ...
    %             scatter(x0,y0,'bx'); hold on;
            ...
            for j = 1:Nc
                ...
                flag_range = 1;
                if abs(rc-r) < delr/sqrt(1-(L^2/(4*Kappas(k)^2)))
                    flag_range=0;
                end
...
Pdc = zeros(length(Kappas),length(rho));
SNR = Pdc;
...
for q = 1:length(rho)
    ...
    for i = 1:length(Kappas)
        ...
        SCR_Nr = threshold*rho(q)*c*tau*(kappa_m^2)*sigma_c_avg;
...
legend({'\rho_c = 0 (M.C.)','\rho_c = 0.001/m^2 (M.C.)','\rho_c = 0.005/m^2 (M.C)','\rho_c = 0 (S.G)','\rho_c = 0.001/m^2 (S.G)','\rho_c = 0.005/m^2 (S.G)'},'Location','southwest','NumColumns',2);

% saveas(gcf,'PdvsKappa_rho','fig');
% saveas(gcf,'PdcvsKappa_rho','png');
...
% ylabel('Bistatic radar detection coverage probability','Fontsize',14,'FontWeight','bold');
legend({'\rho_c = 0','\rho_c = 0.001/m^2','\rho_c = 0.005/m^2'},'Location','southwest','NumColumns',2);

saveas(gcf,'ResultsvsKappa_rho','fig');
saveas(gcf,'ResultsvsKappa_rho','png');