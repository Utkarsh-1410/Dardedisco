...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Pd = zeros(length(Kappas),length(P_tx));
Throughput = zeros(length(Kappas),length(P_tx));
...
for q = 1:length(Kappas)
    ...
    for k = 1:length(P_tx)
        ...
        for i = 1:i_max
            ...
            % received power
            S = (P_tx(k)*H0*sigma_m*B0*epsilon)/(kappa_ch^4);
            % beta
    ...
    %             scatter(x0,y0,'bx'); hold on;
            ...
            for j = 1:Nc
                ...
                if flag_beamwidth==0
                    ...
                    % received power
                    C = C + (P_tx(k)*H0*sigma_c*B0*epsilon)/(R_tc^2*R_rc^2);          
                    n1=n1+1;
...
plot(P_tx,Pd(1,:),'rv-','LineWidth',2,'MarkerSize',10);hold on;
plot(P_tx,Pd(2,:),'ro-','LineWidth',2,'MarkerSize',10);hold on;
plot(P_tx,Pd(3,:),'r*-','LineWidth',2,'MarkerSize',10);hold on;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% STOCHASTIC GEOMETRY FORMULATIONS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Pdc = zeros(length(Kappas),length(P_tx));
Throughput = zeros(length(Kappas),length(P_tx));
SNR = Pdc;
...
for q = 1:length(Kappas)
    ...
    for i = 1:length(P_tx)
        ...
        SNR_Dr = P_tx(i)*H0*sigma_m_avg*B0*epsilon;
...
plot(P_tx,Pdc(1,:),'bv-','LineWidth',2,'MarkerSize',10); hold on;
plot(P_tx,Pdc(2,:),'bo-','LineWidth',2,'MarkerSize',10); hold on;
plot(P_tx,Pdc(3,:),'b*-','LineWidth',2,'MarkerSize',10); hold on;
grid on;
set(gca,'Fontsize',16);
set(gca,'Ylim',[0 1]);
xlabel('Transmitted power $P_{tx}$ (Watts)','interpreter','latex','Fontsize',20,'FontWeight','bold');
...
% saveas(gcf,'PdvsPtx_Kappa','fig');
% saveas(gcf,'PdcvsPtx_Kappa','png');

% figure(2);
subplot(1,2,2);

plot(P_tx,Throughput(1,:),'rv-','LineWidth',2,'MarkerSize',10);hold on;
plot(P_tx,Throughput(2,:),'bo-','LineWidth',2,'MarkerSize',10);hold on;
plot(P_tx,Throughput(3,:),'k*-','LineWidth',2,'MarkerSize',10);hold on;
grid on;
set(gca,'Fontsize',16);
xlabel('Transmitted power $P_{tx}$ (Watts)','interpreter','latex','Fontsize',20,'FontWeight','bold');
...
saveas(gcf,'ResultsvsPtx_Kappa','fig');
saveas(gcf,'ResultsvsPtx_Kappa','png');